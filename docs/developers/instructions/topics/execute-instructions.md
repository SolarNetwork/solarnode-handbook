...
| `instructions` | A JSON array of nested instruction objects. Consult the documentation for each instruction topic you want to use for the syntax required. Note that the `nodeId` property is **not** needed on these instructions, as it is assumed to be the ID of the node executing the `ExecuteInstructions` instruction. The nested instructions **cannot be idependently deffered**, and will ignore any provided `executionDate` parameter. |
